...
const Bracket64SplitInteractive = forwardRef(function Bracket64SplitInteractive(
  ...
  useEffect(() => {
    ...
    window.bracket = {
      ...
      wonGeneric: (side, r, i, pi) =>
        setState((s) => wonGenericOp(s, side, r, i, pi)),
    ...
    <div style={styles.container}>
      <div style={styles.grid(rowsPerHalf)}>
        ...
        {roundNamesRight.map((title, r) => (
          <div
            key={`rh-${r}`}
            style={styles.roundHeader(COLS - r, HEADER_ROWS)}
          >
            {title}
          </div>
        ))}
          ...
          return (
            <>
              ...
              <div style={styles.matchWrap(colStart, rowStart)}>
                <div className="bracket-card" tabIndex={0}>
                  <div className="bracket-name" style={{ textAlign: "center" }}>
                    FINAL
                  </div>

                  <div className="bracket-name" style={{ textAlign: "start" }}>
                    {left}
                  </div>
                  <div className="bracket-name" style={{ textAlign: "end" }}>
                    {right}
                  </div>
                </div>
              </div>
