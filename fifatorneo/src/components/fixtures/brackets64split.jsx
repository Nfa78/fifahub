...
function Connectors({ leftRounds, rightRounds, rowsTotal }) {
    ...
    for (let r = 0; r < rightRounds.length - 1; r++) {
        ...
        for (let i = 0; i < curr.length; i++) {
            const srcPos = gridPlacementHalf({
                r, i, colBase: COLS, dir: -1, // <-- start at far right column 11
            });
            const dstPos = gridPlacementHalf({
                r: r + 1, i: Math.floor(i / 2), colBase: COLS, dir: -1,
            });
    ...
    const leftFinalPos = gridPlacementHalf({ r: LEFT_COLS - 1, i: 0, colBase: 1, dir: +1 }); // col 5
    ...
    const rightFinalPos = gridPlacementHalf({
        r: RIGHT_COLS - 1, i: 0, colBase: COLS, dir: -1, // finalist at col 7
    });
    {
                ...
                {rightRounds.map((matches, r) =>
                    matches.map((m, i) => {
                        const { colStart, rowStart } = gridPlacementHalf({
                            r, i, colBase: COLS, dir: -1, // start at far right, move left to center
                        });
                        return (
